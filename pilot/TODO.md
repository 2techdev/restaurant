...
- [x] **Promotions — Happy Hour backend + schema enrichment** (2026-05-11, code only; deploy queued) — Recon: backoffice `/promotions` zaten 3 alt sayfa ile live (Campaigns + Discounts canlı, Happy Hour localStorage stub). POS Go'da 11 endpoint mevcut. Bu cycle: migration 030 (`discounts_enrich`: HAPPY_HOUR type + 9 yeni kolon + 2 index), Go handler enrichment (Discount struct 9 alan, scanDiscount 24 col), happy-hour-client.tsx tamamen rewritten (localStorage → TanStack Query → `/discounts` filtered by `hours_from && hours_to`), TR i18n +3 anahtar. Commit `36af7f9` üzerinde. **Deploy bloker**: yerel Go/docker yok, sandbox compile edemedi; sonraki cycle Go-capable makineden build+SFTP+migration+restart. Detay: `pilot/DEPLOY_LOG_2026-05-09.md`.
