...
- [~] Reservation entegrasyonu (gastro2hub + backoffice) — D Strategy Aşama 1 (magic-link menu import) **POS Go server tarafı canlı** (2026-05-08, image `magic-link-20260508-230258`, migration 023, endpoint `POST /api/v1/menu/import-from-token` → 401 auth gating OK). Backoffice wizard sayfası canlı (paralel agent, 23:02). Reservation token üretici + by-token consumer Hetzner deploy bekliyor → uçtan uca o zaman aktif. Aşama 2 (modifier CRUD POS'a) sırada.
