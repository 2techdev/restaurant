...
void main() {
  ...
  group('Temel çıktı', () {
    test('build() boş olmayan byte dizisi döndürür', () {
      final bytes = KitchenTicketBuilder(data: makeData()).build();
  ...
  group('Başlık bölümü', () {
    test('Masa numarası yazdırılır', () {
      final text = extractText(KitchenTicketBuilder(data: makeData(tableNo: '12')).build());
      expect(text, contains('12'));
    });

    test('Bon numarası yazdırılır', () {
      final text =
          extractText(KitchenTicketBuilder(data: makeData(orderNo: '0099')).build());
      expect(text, contains('0099'));
    });

    test('Yazıcı grubu büyük harfle yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(data: makeData(printerGroup: 'Kueche')).build(),
      );
      expect(text, contains('KUECHE'));
    });

    test('Yazıcı grubu yoksa grup adı yazdırılmaz', () {
      final text = extractText(KitchenTicketBuilder(data: makeData()).build());
    ...
    test('Garson adı yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(data: makeData(waiterName: 'Anna')).build(),
      );
    ...
    test('Zaman damgası HH:MM:SS formatında yazdırılır', () {
      ...
      final text = extractText(KitchenTicketBuilder(data: makeData(dateTime: dt)).build());
  ...
  group('Ürün bölümü', () {
    test('Ürün adı yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(
          data: makeData(items: [makeItem(name: 'Wiener Schnitzel')]),
        ).build(),
      );
    ...
    test('Adet bilgisi "Nx" formatında yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(
          data: makeData(items: [makeItem(name: 'Pizza', quantity: 3)]),
        ).build(),
      );
    ...
    test('Kesirli adet doğru formatlanır', () {
      final text = extractText(
        KitchenTicketBuilder(
          data: makeData(items: [makeItem(name: 'Steak', quantity: 0.5)]),
        ).build(),
      );
    ...
    test('Modifikatörler ">> " önekiyle yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(
          data: makeData(
            items: [
              makeItem(modifiers: ['ohne Pommes', 'Salat extra']),
            ],
          ),
        ).build(),
      );
    ...
    test('Not "! " önekiyle yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(
          data: makeData(items: [makeItem(notes: 'Sehr scharf bitte')]),
        ).build(),
      );
  ...
  group('İptal (STORNO)', () {
    test('isVoid=true kalemde STORNO yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(
          data: makeData(items: [makeItem(isVoid: true)]),
        ).build(),
      );
    ...
    test('STORNO kalemi adı hâlâ yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(
          data: makeData(items: [makeItem(name: 'Gulasch', isVoid: true)]),
        ).build(),
      );
  ...
  group('Kurs etiketi', () {
    test('courseLabel yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(
          data: makeData(courseLabel: 'Gang 1 - Vorspeise'),
        ).build(),
      );
  ...
  group('Genel sipariş notu', () {
    test('Sipariş notu "NOT:" etiketi ile yazdırılır', () {
      final text = extractText(
        KitchenTicketBuilder(
          data: makeData(notes: 'Allergiker am Tisch!'),
        ).build(),
      );
